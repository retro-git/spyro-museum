...
function parseWADHeader(buffer) {
    const subfiles = [];
...
function parseLVLHeader(buffer) {
    const subfiles = [];
    const entrySize = 8; // Each subfile has 2 integers (offset and size)
    for (let i = 0; i < 10; i++) {
        const offset = readInt32LE(buffer, i * entrySize);
...
async function extractSubfile(inputBuffer, subfile, outputPath) {
...
async function main() {
    ...
    try {
        ...
        console.error(`Failed to open WAD file at ${wadPath}: ${err.message}`);
        process.exit(1);
    }
